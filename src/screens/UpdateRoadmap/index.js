...
export function updateInitRoadmap({route, navigation}) {

const roadmap = route?.params;
console.log(roadmap.levels.size);
  const [label, setLabel] = useState(roadmap.name);
  ...
  const handleNavigation = (route, param1, param2, param3, param4) => {
    navigation?.navigate('SingleStack', {screen: route, params: {roadmapLabel: param1, levels: param2, tags: param3, roadmap: param4}});
  };
  ...
  return (   
    ...
    <View style={styles.seContainer}>
      ...
      <View style={styles.seContainer}>
        ...
        <View style={styles.createButton}><TouchableOpacity onPress={() => handleNavigation('updateRoadmap', label, level, TAGS, roadmap)}>
        <LinearGradient
        start={{ x: 0, y: 0 }}
        end={{x: 1, y: 1 }}
